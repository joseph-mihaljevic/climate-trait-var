...
#-------------------------------------------------------
#-------------------------------------------------------
# Equation for beta function of temp: 
...
# PLOT THE CV-TEMP RELATIONSHIP
CV_plot_func = function(CV_t, CV_piece, T_seq){
  ...
  Temp_time_plot =
    ...
    scale_x_continuous(limits = c(0,max(time_seq)), breaks = c(0,30,60,90)) +
    theme_classic() +
    labs(x = "Day of Annual Epizootic", y = expression("Temp. ("*degree*"C)")) +
    theme(
      axis.text = element_text(color = "black", size = 12),
      axis.title = element_text(color = "black", size = 13),
      strip.text = element_text(color = "black", size = 12)
  ...
  plot_CV_temp_combo = 
    arrangeGrob(CV_temp_plot, Temp_time_plot, CV_plot,
                layout_matrix = matrix(c(1,2,3,3), byrow = T))
  
  return(plot_CV_temp_combo)
...
run_full_sim = function(time_seq){
...
multi_gen_plot = function(annual_store){
  {
    multi_gen_df = 
    ...
    multi_gen_plot = 
      ...
      annotate("segment", x = 0, xend = dim(annual_store)[1],
  ...
  plot_combo = 
    arrangeGrob(multi_gen_plot,
                layout_matrix = matrix(c(2,2,
                                         2,2), 
                                       nrow = 2, byrow = T))
